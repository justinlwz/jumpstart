...
class SpriteSVG extends PureComponent {
  ...
  _parseSVGSprite = (src) => {
    const { defsInjectionSelector } = this.props
    ...
    if (defs) {
      const defsWrapper = document.createElementNS(
  ...
  render() {
    // Props
    const { vBoxWidth, vBoxHeight, icon } = this.props

    // Create Element
    return (
      <svg
        ref={(insertionPoint) => {
          this.spriteSheetInsertion = insertionPoint
        }}
